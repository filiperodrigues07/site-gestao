...
import { Trophy, FileText, Video as VideoIcon } from "lucide-react";
import { db } from "@/lib/db/client";
import { articles, videos } from "@/lib/db/schema";
...
export default async function AdminRankingPage() {
  const [userRows, articleCounts, videoCounts] = await Promise.all([
  ...
  const ranking = userRows
    .map((user) => {
      ...
      return { user, articleCount, videoCount, total: articleCount + videoCount };
    })
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Quem mais publicou artigos e vídeos na Base de Conhecimento — usado para a disputa de
        premiação do suporte.
      </p>
