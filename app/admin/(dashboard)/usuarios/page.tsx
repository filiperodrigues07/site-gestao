...
export default async function AdminUsuariosPage({
  ...
  searchParams: Promise<{ q?: string; acesso?: string; page?: string }>;
}) {
  await requireAdmin();
  const { q, acesso, page: pageParam } = await searchParams;
  ...
  const buildHref = (targetPage: number) => buildAdminHref("/admin/usuarios", { q, acesso }, targetPage);
    ...
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{total} usuários cadastrados</p>
        </div>
